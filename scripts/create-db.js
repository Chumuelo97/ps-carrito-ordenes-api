...
async function run() {
  ...
  const user = process.env.DB_USER || process.env.DB_USERNAME || process.env.DB_USER_NAME || 'root';
  // support multiple common env names for password
  const password = process.env.DB_PASSWORD || process.env.DB_PASS || process.env.DB_PASSWD || '';
  const db = process.env.DB_NAME || process.env.DB_DATABASE || process.env.DB_DB || 'ps_carrito';
